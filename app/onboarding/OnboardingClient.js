...
export default function Onboarding() {
  ...
  useEffect(() => {
mkdir -p app/onboard && cat > app/onboard/page.js << 'EOF'
'use client';
import { useState, useEffect } from 'react';
import { createClient } from '@supabase/supabase-js';
    ...
    <div style={{ display:'flex', justifyContent:'center', alignItems:'center', minHeight:'100vh', background:'#f0f9f5' }}>
      <div style={{ width:320, background:'#fff', borderRadius:20, padding:'2rem', border:'0.5px solid #e0e0e0' }}>
        ...
        <input
          ...
          style={{ width:'100%', padding:'10px 12px', borderRadius:10, border:'0.5px solid #ddd', fontSize:14, marginBottom:12, fontFamily:'inherit' }}
          autoFocus
        />
        <button
          onClick={handleSave}
          disabled={loading || !name.trim()}
          style={{ width:'100%', padding:12, borderRadius:12, background: !name.trim() ? '#ccc' : '#1D9E75', color:'#fff', border:'none', fontSize:14, fontWeight:600, cursor: !name.trim() ? 'default' : 'pointer', fontFamily:'inherit' }}
        >
